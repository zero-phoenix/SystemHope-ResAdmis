...
5. **Espaciado y Párrafos (¡MUY IMPORTANTE!):** El documento es de interlineado simple (`1.0`). Para separar párrafos, se inserta una línea en blanco (un párrafo vacío). Se debe configurar el `SpaceBefore` y `SpaceAfter` en 0 puntos para no añadir espaciado oculto, controlando los saltos de línea manualmente para que todo calce exactamente en 6 páginas. Las viñetas numéricas y alfabéticas deben usar sangría izquierda y sangría francesa para que el texto justificado se alinee bajo sí mismo y no vuelva al margen izquierdo absoluto.
6. **Citas y Notas al Pie (¡EL SECRETO DE LAS 6 PÁGINAS!):** Siempre que se invoque una norma, debe insertarse una nota al pie real de Microsoft Word (Footnote). La inyección técnica debe ser perfecta. La fuente de las notas al pie es estrictamente **Arial Narrow tamaño 8.0**, alineación **Justificada**. Un tamaño mayor (ej. 10.0) inflará el documento de 6 a 7 o más páginas, lo cual es inaceptable.
