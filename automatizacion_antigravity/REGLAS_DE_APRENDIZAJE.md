...
Encabezado en Cuerpo (Header-Body): La frase "SECRETARIA TÉCNICA DE LA COMISIÓN DE PROTECCIÓN AL CONSUMIDOR 1 SEDE CENTRAL" debe ir alineada a la derecha, en cursiva y negrita en la parte superior del documento (después del Header con imágenes).
Cuerpo de Metadatos (Sin Tablas): Los datos iniciales (EXPEDIENTE :, DENUNCIANTE :, etc.) NO deben construirse como una tabla de Word. Deben usar sangría francesa (Hanging Indent de -1.48 pulgadas) y tabulación para alinear el valor a la derecha. El campo va en texto normal y el valor en negrita.
