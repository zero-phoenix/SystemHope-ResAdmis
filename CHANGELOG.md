...
### Fixed
- **R-212** aceptaba una cita de traslado incompleta si era subcadena de la admisión: una denuncia sin su subsanación pasaba el control. Se compara la cita completa y se amplían las mutaciones de la misma regla.
- **`entregar`** buscaba las fechas de los escritos en PRIMERO aunque la admisión estuviera en otro ordinal. Ahora identifica el ordinal por la admisión; no obliga a borrar o desplazar indebidamente inadmisibilidad o confidencialidad.
- AGENTS, skills, generador de la skill de imputaciones, reglas de aprendizaje y README usan «ordinal de admisión». No se introduce otra regla ni se alteran datos de expedientes.

### Added
- `scripts/prueba_traslado.py`: reproducción de la omisión, citas completas, admisión en SEGUNDO/TERCERO, conservación de ordinales previos e idempotencia; se ejecuta en CI.
- Medición del corpus: 574 plantillas; admisión en PRIMERO 516, SEGUNDO 57, TERCERO 1; 252 citas compuestas y 2 plantillas que dejan sin efecto inadmisibilidad. Resolución documentada en `docs/CONTRADICCIONES_RESUELTAS.md`.
...
- **R-212 (traslado)**: «correr traslado de la denuncia del …[, subsanada mediante escrito del …] a …», con la misma cita del artículo que admite la denuncia. Falsador (rechaza «de la presente resolución» y la cita que difiera del ordinal de admisión) y mutación; el control de omisiones se corrige en v3.5.1.
...
- **Plantillas maestras**:
  ...
  - Las 574 plantillas corren traslado de la denuncia, con la cita copiada de su ordinal de admisión.
...
- **`sanear_admisorio.py`**: el constructor añade la frase de R-211 y pone o sincroniza la cita del traslado con el ordinal de admisión del caso, que antes heredaba las fechas de la plantilla.
